...
def parse_arguments():
    description = 'Arguments to obtain a list of files for triggering the Inactive IAM Users job'
...
def get_list_of_files_from_s3(folder_path, s3_bucket_name, s3_client):
    try:
        ...
        logging.info(f'Attempting to obtain list of files from path {folder_path} in {s3_bucket_name}')
        objects_in_path = s3_client.list_objects(Bucket=s3_bucket_name, Prefix=folder_path)
        for object in objects_in_path["Contents"]:
            list_of_files_from_s3.append(object["Key"])
        for file in list_of_files_from_s3:
            if file == folder_path:
                logging.debug(f'Removing returned object that matches folder path {folder_path}')
            list_of_files_from_s3.remove(file)
        logging.info("List of files successfully obtained")
        return list_of_files_from_s3
    except botocore.exceptions.ClientError as e:
        logging.error(f'Unable to obtain list of files in {folder_path} path for {s3_bucket_name}: {e}')
        exit(1)
...
def download_from_s3():
    ...
    list_of_files_from_s3 = get_list_of_files_from_s3(folder_path=folder_path, s3_bucket_name=s3_bucket_name, s3_client=s3_client)
